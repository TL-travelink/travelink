...
const loadHotBoard = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/hot-board`, {
      headers: {
...
const hotUsers = ref([])
const loadHotUser = async () => {
  try {
    const response = await axios.get(`${import.meta.env.VITE_API_URL}/member/best`, {
      headers: {
        Authorization: `Bearer ${sessionStorage.getItem('accessToken')}`
      }
    });
...
<template>
    ...
    <div class="pt-24 mt-24">
      <div class="container px-3 mx-auto flex flex-wrap flex-col md:flex-row items-center justify-center">
        <!--Left Col-->
        <div
          class="flex flex-col w-full md:w-2/5 justify-center items-center text-center md:text-left"
        >
          <h1 class="my-4 text-5xl font-bold leading-tight">여행의 시작</h1>
          <p class="leading-normal text-2xl mb-8">Travelink에서 함께하세요!</p>
          <RouterLink :to="{name:'map'}" class="mx-auto lg:mx-0 hover:underline bg-white text-gray-800 font-bold rounded-full my-6 py-4 px-8 shadow-lg focus:outline-none focus:shadow-outline transform transition hover:scale-105 duration-300 ease-in-out z-50">
            Let's Go !!
          </RouterLink>
        </div>
        <!--Right Col-->
<!--        <div class="w-full md:w-3/5 py-6 text-center">-->
<!--          <img class="w-full md:w-4/5 z-50" src="@/assets/giphy.gif" />-->
<!--        </div>-->
      </div>
    </div>
      ...
      <div class="container max-w-5xl mx-auto m-8">
        ...
        <div class="flex flex-wrap">
          <div class="w-5/6 sm:w-1/2 p-6">
            <h3 class="text-3xl text-gray-800 font-bold leading-none mb-3">친구와 함께</h3>
        ...
        <div class="flex flex-wrap flex-col-reverse sm:flex-row">
          <div class="w-full sm:w-1/2 p-6 mt-6">
            <img class="w-5/6 sm:h-64 mx-auto" src="@/assets/guide-video.gif" />
          </div>
          <div class="w-full sm:w-1/2 p-6 mt-6">
    ...
    <section class="bg-white border-b py-8">
      <div class="container mx-auto flex flex-wrap pt-4 pb-12 justify-center">
        <h2 class="w-full my-2 text-5xl font-bold leading-tight text-center text-gray-800">Hot Board</h2>
        <div class="w-full mb-4">
          <div class="h-1 mx-auto gradient w-64 opacity-25 my-0 py-0 rounded-t"></div>
        </div>
        <BoardComponent v-for="board in hotBoardList" :board="board"/>
      </div>
    </section>
    <section class="bg-white py-8">
      <div class="container mx-auto px-2 pt-4 pb-12 text-gray-800">
        ...
        <div class="flex">
<!--          <img src="@/assets/crown.jpg" class="text-center"/>-->
          <h2 class="w-full my-2 text-5xl font-bold leading-tight text-center text-gray-800">
            우수 회원
          </h2>
        </div>
        ...
        <div class="flex flex-col sm:flex-row justify-center pt-12 my-12 sm:my-4">
          <UserComponent v-for="(user, index) in hotUsers" :user="user" :index="index"/>
        </div>
      </div>
    </section>
